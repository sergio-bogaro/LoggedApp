...
function CharactersTab({ characters }: { characters: CharacterNode[] }) {
  ...
  if (!characters.length) {
    return <p className="text-sm text-muted-foreground py-4">{t("animeTabs.empty.characters")}</p>;
  }
...
function StaffTab({ staff }: { staff: StaffEdge[] }) {
  ...
  if (!staff.length) {
    return <p className="text-sm text-muted-foreground py-4">{t("animeTabs.empty.staff")}</p>;
  }
...
function RelationsTab({ relations }: { relations: RelationEdge[] }) {
  ...
  if (!filtered.length) {
    return <p className="text-sm text-muted-foreground py-4">{t("animeTabs.empty.relations")}</p>;
  }
